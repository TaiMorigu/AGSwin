...
Enemy::Enemy(GameScene* gs,const Vector2& playerPos):
	gInst(gs),
	playerPos(playerPos)
{
}
...
void Enemy::Load(void)
{
	...
	std::string path = "Data/Image/object/Enemy/";
	path += imgFName;
...
void Enemy::Init(void)
{
	...
	Utility::DIRECTION eDir = static_cast<Utility::DIRECTION>(dir);
	Vector2I rpos = gInst->GetLpStage()->GetMapDispStPos();
	rpos.x *= StageBase::MAP_CHIP_SIZE_WID;
	rpos.y *= StageBase::MAP_CHIP_SIZE_HIG;

	switch (eDir) {
	case Utility::DIRECTION::E_DIR_UP:
		rpos.x += GetRand(Application::SCREEN_SIZE_X - 1);
		rpos.y += Application::SCREEN_SIZE_Y;
		break;
	case Utility::DIRECTION::E_DIR_DOWN:
		rpos.x += GetRand(Application::SCREEN_SIZE_X - 1);
		break;
	case Utility::DIRECTION::E_DIR_LEFT:
		rpos.y += GetRand(Application::SCREEN_SIZE_Y - 1);
		rpos.x += Application::SCREEN_SIZE_Y;
		break;
...
void Enemy::Update(void)
{
	...
	// ベクトル(相手－自分)
	Vector2 vec = Vector2(static_cast<float>(pPos.x) - pos.x,
...
{
	for (int yy = static_cast<int>(Utility::DIRECTION::E_DIR_MAX); yy > 0; yy--) {
		for (int xx = ANIM_NUMS; xx > 0; xx--) {
			if (DeleteGraph(img[yy - 1]) == -1) {
...
void Enemy::SetMoveDirection(Vector2 edir)
{
	if (abs(edir.x) < abs(edir.y)) {
		// 上下
		if (edir.y > 0.0f) {
			dir = static_cast<int>(Utility::DIRECTION::E_DIR_DOWN);
		}
		else {
			dir = static_cast<int>(Utility::DIRECTION::E_DIR_UP);
		}
	}
	else {
		// 左右
		if (edir.x > 0.0f) {
			dir = static_cast<int>(Utility::DIRECTION::E_DIR_RIGHT);
		}
		else {
			dir = static_cast<int>(Utility::DIRECTION::E_DIR_LEFT);
		}
	}
}
