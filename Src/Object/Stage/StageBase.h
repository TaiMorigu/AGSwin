...
public:
	...
	void Load(void); // 初期化処理(最初の１回のみ実行)
	void Init(void); // ゲーム起動・再開時に必ず呼び出す処理
	void Update(void); // 更新処理
	void Draw(void); // 描画処理
	void Release(void); // 解放処理(最後の１回のみ実行)



	Vector2I GetMapDispStPos(void) { return mapDispStPos; }
	int GetMapChipNo(Vector2I mPos) { return dispMapDat[mPos.y][mPos.x]; }
	Vector2I GetDispMapSize(void) { return dispMapSize; }
	StageBase::MAP_TYPE GetMapType(void) { return mapType; }
	bool IsStair(Vector2I mapPos) {
...
private:
	...
	Vector2I mapDispStPos; // マップ表示開始座標 
